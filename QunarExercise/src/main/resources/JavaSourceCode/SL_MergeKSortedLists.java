 ...
 *
 */
public class SL_MergeKSortedLists {
	/**
	 * 链表结构
	 * @author maoge
	 *
	...
	/**
	 * 复用合并两个链表的函数，两个链表合成一个新链表之后再和下一个链表合并
	 * @param list
	 * @return
	 */
	...
	public static void main(String[] args) {
		SL_MergeKSortedLists sl = new SL_MergeKSortedLists(); 
		...
		//初始化链表
		list3[0] = sl.new ListNode(3);
		list3[1] = sl.new ListNode(4);
		list3[2] = sl.new ListNode(5);
		list3[3] = sl.new ListNode(7);
		list3[4] = sl.new ListNode(11);
	
		//构造链表
		for(int i=0 ; i < 5 ; i++){
			if( 4 == i)
				list3[4].next = null ;
			else list3[i].next = list3[i+1] ;
		}
		...
		ListNode mMerge = sl.mergeKLists(test);
